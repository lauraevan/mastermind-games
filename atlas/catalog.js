...
(function (Catalog) {
    ...
    function shift(s) {
        ...
        for (let i = 0; i < s.length; i++) {
            ...
            if (c >= 65 && c <= 90)
                out += String.fromCharCode(((c - 65 + 7) % 26) + 65);
            else if (c >= 97 && c <= 122)
                out += String.fromCharCode(((c - 97 + 7) % 26) + 97);
            else if (c >= 48 && c <= 57)
                out += String.fromCharCode(((c - 48 + 3) % 10) + 48);
            else
                out += s.charAt(i);
        }
        return out;
    ...
    function loadData(cb) {
        ...
        sc.src = 'data.js';
        sc.onload = () => { ingest(); cb(); };
        sc.onerror = () => { const l = byId('vLoading'); if (l)
            l.textContent = 'Could not reach the archive.'; };
    ...
    function updateCount() {
        const c = byId('count');
        if (c)
            c.textContent = state.filtered.length.toLocaleString() + ' entries';
    }
    function card(r) {
    ...
    function renderMore() {
        ...
        if (more)
            more.style.display = state.shown < state.filtered.length ? 'inline-block' : 'none';
    }
    // ---------- overlay ----------
    function open(r) {
        const ov = q('#ov');
        const frame = q('#ovFrame');
        const title = q('#ovTitle');
    ...
    function reveal() {
        ...
        if (!state.loaded) {
            const l = byId('vLoading');
            if (l)
                l.style.display = 'block';
            loadData(() => { const l2 = byId('vLoading'); if (l2)
