...
(function (Catalog) {
    ...
    function renderRecent() {
        const wrap = byId('recentWrap');
        const row = byId('recentRow');
        if (!wrap || !row)
            return;
    ...
    function init() {
        ...
        if (ovFull)
            ovFull.addEventListener('click', () => {
                const stage = q('.ov-stage');
                const el = (stage || q('#ov'));
        ...
        if (search && !('ontouchstart' in window) && window.innerWidth > 760) {
            try {
                search.focus();
            }
            catch (e) { }
