...
(function (global) {
  ...
  // Synthesize a worked explanation for a problem that didn't ship one, by
  // recognizing common arithmetic shapes in the prompt. Falls back gracefully.
  function explainFor(q, answer) {
    var a = String(answer);
    if (q == null) return 'The correct answer is ' + a + '.';
    var core = String(q).replace(/\([^)]*\)\s*$/, '').replace(/\s+/g, ' ').trim();
    var m;
    if ((m = core.match(/√\s*(\d+(?:\.\d+)?)/)))
      return 'A square root asks what number times itself gives ' + m[1] + '. Since ' + a + ' × ' + a + ' = ' + m[1] + ', the answer is ' + a + '.';
    if ((m = core.match(/(\d+(?:\.\d+)?)\s*²/)))
      return m[1] + ' squared means ' + m[1] + ' × ' + m[1] + ', which equals ' + a + '.';
    if ((m = core.match(/(\d+(?:\.\d+)?)\s*%\s*of\s*(\d+(?:\.\d+)?)/i)))
      return 'To find ' + m[1] + '% of ' + m[2] + ', multiply ' + m[2] + ' by ' + m[1] + ' ÷ 100. That gives ' + a + '.';
    if ((m = core.match(/(\d+)\s*\/\s*(\d+)\s*\+\s*(\d+)\s*\/\s*(\d+)/)) && m[2] === m[4])
      return 'The denominators match, so add the numerators: (' + m[1] + ' + ' + m[3] + ') over ' + m[2] + ' = ' + a + '.';
    if ((m = core.match(/^(-?\d+(?:\.\d+)?)\s*([+\-−×x*·÷\/])\s*(-?\d+(?:\.\d+)?)$/))) {
      var op = m[2], x = m[1], y = m[3];
      if (op === '+') return 'Add the two numbers: ' + x + ' + ' + y + ' = ' + a + '.';
      if (op === '-' || op === '−') return 'Subtract: ' + x + ' − ' + y + ' = ' + a + '.';
      if (op === '×' || op === 'x' || op === '*' || op === '·') return 'Multiply: ' + x + ' × ' + y + ' = ' + a + '.';
      if (op === '÷' || op === '/') return 'Divide: ' + x + ' ÷ ' + y + ' = ' + a + '.';
    }
    if ((m = core.match(/x\s*=|solve/i)))
      return 'Isolate the variable step by step to find ' + a + '.';
    return 'The correct answer is ' + a + '.';
  }

  // Base skill name without the "· Level N" suffix.
  function baseName(n) { return String(n).replace(/\s*·\s*Level\s*\d+\s*$/, ''); }
  ...
  (function () {
    ...
    var seq = 0;
    var LX = 7; // level multiplier — scales per-skill difficulty tiers to reach an IXL-sized catalog
    ...
    // ----- interactive item types (number line, select-all) -----
    addFam('math', 'Interactive', 'Plot a number on the number line', [1, 7], function (g, lv) {
      return function () {
        var max = [10, 20, 20, 50, 100, 100, 200][Math.min(lv - 1, 6)];
        var step = max > 20 ? (max >= 100 ? 10 : 5) : 1;
        var t = rnd(1, Math.floor(max / step)) * step;
        return { type: 'nl', q: 'Plot ' + t + ' on the number line.', min: 0, max: max, step: step, answer: String(t), explain: t + ' is ' + t + ' units to the right of 0.' };
      };
    }, 1);
    addFam('math', 'Interactive', 'Select all that apply', [2, 8], function (g, lv) {
      return function () {
        var kinds = ['even numbers', 'odd numbers', 'multiples of 3', 'multiples of 5', 'numbers greater than 10'];
        var kind = pick(kinds);
        function test(v) { return kind === 'even numbers' ? v % 2 === 0 : kind === 'odd numbers' ? v % 2 === 1 : kind === 'multiples of 3' ? v % 3 === 0 : kind === 'multiples of 5' ? v % 5 === 0 : v > 10; }
        var pool = []; while (pool.length < 6) { var v = rnd(1, 12 + lv * 6); if (pool.indexOf(v) === -1) pool.push(v); }
        var ans = pool.filter(test);
        if (!ans.length) { pool[0] = kind === 'even numbers' ? 2 : kind === 'odd numbers' ? 3 : kind === 'multiples of 3' ? 6 : kind === 'multiples of 5' ? 10 : 11; ans = pool.filter(test); }
        return { type: 'multi', q: 'Select all the ' + kind + ':', choices: pool.map(String), answers: ans.map(String), explain: 'The ' + kind + ' here are: ' + ans.join(', ') + '.' };
      };
    }, 1);
  ...
  var Problems = {
    ...
    generate: function (skillId, grade, bump) {
      var k = SKILL_INDEX[skillId]; if (!k) return null;
      // Adaptive: when the learner is on a run, serve harder problems from a
      // higher level of the SAME skill — mastery still counts for skillId.
      var genK = k;
      if (bump) { var sib = this.levelSibling(skillId, bump); if (sib) genK = sib; }
      var p = genK.gen(grade || 'K');
      p.skillId = skillId; p.subjectId = k.subject; p.skillName = k.name;
      if (!p.explain) p.explain = explainFor(p.q, p.answer);
      return p;
    },
    // The same skill at a higher (+) or lower (−) difficulty level, if one exists.
    levelSibling: function (skillId, delta) {
      var k = SKILL_INDEX[skillId]; if (!k) return null;
      var base = baseName(k.name), want = (k.level || 1) + delta;
      if (want < 1) return null;
      var subj = null; for (var i = 0; i < SUBJECTS.length; i++) if (SUBJECTS[i].id === k.subject) { subj = SUBJECTS[i]; break; }
      if (!subj) return null;
      for (var j = 0; j < subj.skills.length; j++) {
        var o = subj.skills[j];
        if (o.strand === k.strand && o.grade === k.grade && (o.level || 1) === want && baseName(o.name) === base) return o;
      }
      return null;
    },
